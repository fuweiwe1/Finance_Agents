...
test.describe('股票 Agent 工作台 E2E', () => {
  test('三栏渲染 + 默认 TSLA 详情 + 2×2 卡片 + Tab 切换', async ({ page }) => {
    await page.goto('/');
    ...
    // 默认 TSLA，详情头部显示真实中文名（等待实时数据）
    await expect(page.locator('h1')).toContainText('特斯拉', { timeout: 20_000 });
  ...
  test('选中自选联动详情 + 未配置模型时发送消息出提示', async ({ page }) => {
    ...
    // 切到 AAPL
    await page.getByText('AAPL.US', { exact: true }).click();
    await expect(page.locator('h1')).toContainText('苹果', { timeout: 20_000 });

    // 发消息：未配置模型 → 出现引导提示
    await page.getByPlaceholder('输入问题，Enter 发送').fill('TSLA 现在多少钱');
